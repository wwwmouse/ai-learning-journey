...
train_transform = transforms.Compose([
    transforms.RandomHorizontalFlip(),       # 随机水平翻转（CIFAR-10 才有意义）
...
ax1.set_ylabel('Loss')
ax1.set_title('CIFAR-10 CNN+ Loss (optimized)')

ax2.plot(range(1, epochs+1), test_accs, marker='o', color='green')
ax2.set_xlabel('Epoch')
ax2.set_ylabel('Accuracy (%)')
ax2.set_title('CIFAR-10 CNN+ Accuracy (optimized)')

plt.tight_layout()
plt.savefig(os.path.join(IMAGES_DIR, 'cifar10_cnn_optimized.png'), dpi=150)
plt.show()
print(f"训练曲线已保存到 {IMAGES_DIR}/cifar10_cnn_optimized.png")

# ===== 保存模型 =====
MODELS_DIR = os.path.join(PROJECT_DIR, 'models')
os.makedirs(MODELS_DIR, exist_ok=True)
torch.save(model.state_dict(), os.path.join(MODELS_DIR, 'cifar10_cnn_optimized.pth'))
print(f"模型已保存到 {MODELS_DIR}/cifar10_cnn_optimized.pth")
