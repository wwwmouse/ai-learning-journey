...
def main():
    ...
    # 结果记录
    # 第一版（有 bug）: 46.99%  ← 改 stem 时把 conv1 换成了随机层，而冻结让它永远学不了
    # 修复后          : ??%    ← 把预训练 7x7 插值成 3x3 装回 conv1 之后重跑
    #
    # 为什么第一版那么低（已用只推理的诊断确认）：
    #   冻结版 conv1 与初始值的平均绝对差 = 0.00000000  →  它一动没动，始终是随机权重
    #   可训练参数 5,130 / 11,173,962 (0.05%)          →  5130 = 512x10+10，只有分类头在学
    #   微调版 conv1 与初始值的平均绝对差 = 0.02796428 →  它自己学出来了，所以能到 93.98%
