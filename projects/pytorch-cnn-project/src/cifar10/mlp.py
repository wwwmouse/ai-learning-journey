...
# 定义模型
# 32×32×3=3072 个像素 → 10 个输出（飞机/汽车/鸟/猫/鹿/狗/青蛙/马/船/卡车）
model = nn.Sequential(
    nn.Flatten(),              # 把 [64, 3, 32, 32] 摊平成 [64, 3072]
    nn.Linear(3072, 128),      # 3072 个像素 → 128 个神经元
    nn.ReLU(),
...
ax1.set_ylabel('Loss')
ax1.set_title('CIFAR-10 MLP Training Loss')

# 右图：准确率曲线
ax2.plot(range(1, epochs+1), test_accs, marker='o', color='green')
ax2.set_xlabel('Epoch')
ax2.set_ylabel('Accuracy (%)')
ax2.set_title('CIFAR-10 MLP Test Accuracy')

plt.tight_layout()
plt.savefig(os.path.join(IMAGES_DIR, 'cifar10_mlp.png'), dpi=150)
plt.show()
print(f"训练曲线已保存到 {IMAGES_DIR}/cifar10_mlp.png")

# ===== 保存模型 =====
MODELS_DIR = os.path.join(PROJECT_DIR, 'models')
os.makedirs(MODELS_DIR, exist_ok=True)
torch.save(model.state_dict(), os.path.join(MODELS_DIR, 'cifar10_mlp.pth'))
print(f"模型已保存到 {MODELS_DIR}/cifar10_mlp.pth")
