...
# 定义模型（CNN）
# 对比 MLP：CNN 用 Conv2d 在图上滑动窗口提取特征，保留图片的二维结构
# Conv2d 看局部（3×3 窗口），Linear 看全局（所有像素）
model = nn.Sequential(
    # 第 1 块：提取简单特征（边缘、颜色变化）
    nn.Conv2d(1, 16, kernel_size=3, padding=1),  # 1通道 → 16张特征图，窗口3×3
    nn.ReLU(),
    nn.MaxPool2d(2),                               # 28×28 → 14×14，缩小一半

    # 第 2 块：提取复杂特征（形状、纹理组合）
    nn.Conv2d(16, 32, kernel_size=3, padding=1),  # 16通道 → 32张特征图
    nn.ReLU(),
    nn.MaxPool2d(2),                               # 14×14 → 7×7

    # 分类头：和 MLP 一样，最后用 Linear 做判断
    nn.Flatten(),                                  # [batch, 32, 7, 7] → [batch, 1568]
    nn.Linear(1568, 128),
    nn.ReLU(),
    nn.Linear(128, 10),                            # → 10 个类别分数
)
...
for epoch in range(epochs):
    ...
    # epoch 10: train_loss=0.2303, test_acc=93.80%
...
plt.tight_layout()
plt.savefig(os.path.join(IMAGES_DIR, 'mnist_cnn.png'), dpi=150)
plt.show()
print(f"训练曲线已保存到 {IMAGES_DIR}/mnist_cnn.png")

# ===== 保存模型 =====
MODELS_DIR = os.path.join(PROJECT_DIR, 'models')
os.makedirs(MODELS_DIR, exist_ok=True)
torch.save(model.state_dict(), os.path.join(MODELS_DIR, 'mnist_cnn.pth'))
print(f"模型已保存到 {MODELS_DIR}/mnist_cnn.pth")