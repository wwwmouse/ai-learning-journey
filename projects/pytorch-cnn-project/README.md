...
python src/cifar10/cnn_pretrained.py  # 63.63%  迁移学习①诊断用，见下
python src/cifar10/cnn_finetune.py    # 93.98%  迁移学习②换更好的起点，+13.95
```
...
**它 63.63% 输给从零训练的 80.03%，这不代表迁移学习没用。** 原因是训练预算根本不同
...
```text
第一版（有 bug）：直接 nn.Conv2d(...) 新建 stem 层     → 46.99%  比从头训还差
   ↓ 病因
...
**这个 46.99% → 63.63% 的链条，比 93.98% 那个数字更有教学价值** —— 它演示了
"看到一个坏数字时，怎么从机制上找病因"，而不是猜参数。

### ② 微调版才是"换更好的起点"

微调版的 93.98% 相对从零训练的 80.03% 是 **+13.95**，这是本项目最大的一次单步提升。
