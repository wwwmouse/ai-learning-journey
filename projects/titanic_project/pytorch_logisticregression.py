...
for step in range(2001):
    z=model(x_t).squeeze() # .squeeze() 把形状从 (623,1) 压成 (623,)
    # numpy: z = x@w + b; pred = sigmoid(z)
    
    loss=loss_fn(z,y_t)
    # numpy: loss = -np.mean(y*log(pred) + (1-y)*log(1-pred))
    
    optimizer.zero_grad() # 清空上一轮的梯度（ numpy 里每次循环 dw/db 自动覆盖所以不用清）
    loss.backward() # 自动算 dw 和 db（取代手写的梯度公式）
    optimizer.step() # w -= lr*dw（取代手写的更新）
