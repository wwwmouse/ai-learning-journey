...
def main():
    ...
    print('\n' + '═' * 78)
    print('读法提醒：')
    print('  · P/R/F1 均对齐 pos_label=1（「存活」这一类）')
    print('  · 调阈值和类别加权 = 在同一条 PR 曲线上滑动（只换取舍，不产生信息）')
    print('  · 加「称谓」= 把曲线往外推（准确率和精确率同时上升）')
    print('  · 「家庭规模」是假特征：逻辑回归一位小数都不动，因为它只是 SibSp+Parch 的线性组合')
    print('  · ★ 所有"拧旋钮"的手段，天花板都是 215/268：')
    print('      基线 212 / 调阈值 213 / 类别加权 214 / 堆到 1000 棵 215 / 网格搜索 215 / 加家庭规模 215')
    print('      而加「称谓」一步跳到 220（森林）、224（逻辑回归）—— 唯一离开噪声的手段')
    print('═' * 78)
