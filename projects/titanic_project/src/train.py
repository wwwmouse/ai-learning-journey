...
# 【结论修正 2026-09】原来这里写的是"66% 是这 7 个特征能达到的天花板" —— 那是错的归因。
# 后面又做了四组排查 + 三条路实测（同一份 623/268 切分，测试集全程不参与调参）：
#   ① 收敛：逻辑回归 n_iter_=8 就收敛，max_iter 从 100 加到 10 万结果一字不变 → 优化没欠账
#   ② 容量：决策树越深越差（depth=5 时 0.791，不限深度跌到 0.761）→ 过拟合一侧，容量过剩
#   ③ 规模：随机森林 100→1000 棵树只 +0.011 准确率 → 收益趋零
#   ④ 超参：GridSearchCV 36 组 × 5 折 = 180 次训练，只 +1%
# 三条候选路线：
#   调阈值    → 不产生新信息：只是换切点，把漏判和误判互换。森林 0.791→0.795（召回+2点、F1 几乎不动）
#               逻辑回归更极端：阈值降到 0.30，召回 0.71→0.83，精确率 0.74→0.67，F1 只从 0.73 挪到 0.74
#   类别加权  → 召回 0.71→0.81，但精确率 0.74→0.71 —— 只是在同一条 PR 曲线上滑动
#   加特征    → 随机森林 0.791→0.821、精确率 0.76→0.83；逻辑回归 0.795→0.836 —— 把曲线往外推
# 所以"召回低"不是特征不够（加权就能提），但"让模型真正变强"只有加信息量一条路。
# 注意：只有"增加了信息"的特征才算数 —— 「家庭规模」是 SibSp+Parch 的线性组合，信息量为零，
#       逻辑回归 0.795→0.795 一位小数都不动，森林只小涨到 0.802（少问一层问题而已）。
# 教训：归因"特征不够"之前，先按「收敛 → 容量（含规模）→ 超参 → 评估口径」的顺序排查。
