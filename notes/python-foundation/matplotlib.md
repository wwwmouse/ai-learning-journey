...
这很关键，画布大小直接决定**数据是否会相互碰撞，图的观感是否合适**
...
### 显示图表:
`plt.show()`
它会终结当前画布的生命周期、清空绘图上下文
**这就导致下一次绘图会新建画布并走默认尺寸**
**切记！**
...
plt.bar(subjects,scores,color="orange",width=0.3)
plt.title("各科成绩",color="red",fontsize=20)
plt.xlabel("科目",fontsize=10,color="pink")
plt.ylabel("成绩",fontsize=10,color="green",rotation=0)
