...
sklearn 阶段，泰坦尼克 623 条数据一次全传 `model.fit(X, y)`；MNIST 有 60000 张图（约 47 MB），其实**也装得下内存**。

真正需要 `DataLoader` 的理由有三条，内存只是其中最弱的一条：

1. **小批量梯度下降**——算法层面的硬需求：每个 batch 算一次梯度、更新一次参数。全集梯度下降在 60000 张图上每步太慢，泛化往往也更差；
2. **每个 epoch 打乱顺序**——否则模型可能学到样本的排列顺序；
3. **每个样本现场跑 transform**——数据增强就挂在这里，每轮看到的图都不一样。

将来换到 ImageNet 那种规模（百 GB 级），内存才会变成第四条、也是最硬的理由。
...
1. 从内存缓冲区取出第 0 张图片 → PIL Image（28×28，像素 0~255）
2. 读标签（整数 0~9）
3. 如果 `self.transform` 不为空 → `img = self.transform(img)`
4. 返回 `(img, label)`

每次 `train_data[i]` 都会重新跑一遍 transform（数据在 `__init__` 时已经解码进内存，`__getitem__` 只做切片 + 变换）。这就是为什么 1.1 说 transform 是懒加载、且每次拿到的图都可能不同。
...
**底层运算**：在卷积层输出后、ReLU 激活前，**把数据拉回标准范围**——减均值、除标准差。
参数更新带来的分布漂移，被挡在这一层里，不会一路累积到最后一层。

**重要性**：没有 BatchNorm 时，5 层卷积的 CIFAR-10 准确率直接崩到 **20%**，加了 BN 后一条线没改就跳到 **80%**。

```
无 BN:  输入 → [Conv1] → 输出值偏大 → [Conv2] → 逐层漂移 → [Conv5] → 训练不稳定
有 BN:  输入 → [Conv1] → [标准化：均值0方差1] → [Conv2] → [标准化] → ... → 稳定
```

>**每一层参数更新都会改变它输出的数值范围；在深层网络里这种漂移会逐层累积，让后面层收到的输入分布不断变化，训练因此难以稳定。**

两点补充：

1. **"逐层放大"是 BN 原论文（2015）的解释**。后来的工作（Santurkar et al., 2018）认为是 **BN 平滑了损失曲面**、让梯度更可预测，而不是消除了"内部协变量偏移"。这个争论我还没深读，先按原论文记。
2. **BN 的统计量来自当前 batch**——所以 batch 太小噪声大、效果变差，**`batch_size=1` 时 BN 根本无法工作**，那种场景要用 LayerNorm / GroupNorm。（本项目的 CIFAR-10 用 batch_size=64，恰好落在安全区。）
...
> **加了 BN 后所有参数没动，直接到 80%。BN 是深层网络最常用的稳定手段，但不是唯一手段——残差连接、LayerNorm/RMSNorm、更好的初始化，也都能让深层网络训起来。**
...
| 什么时候加 | 深层网络**通常**需要 BN 或等价的归一化手段| 确定过拟合后才加 |
...
**这就解释了为什么不建议把 MNIST 的 CrossEntropyLoss 换成 BCEWithLogitsLoss**：
不是因为"接口不匹配"——技术上完全可以把 10 个输出每个接一个 sigmoid + BCE，也能训到不错的准确率；但那等于问 10 道独立的是非题，丢掉了"互斥"这个先验，归纳偏置和概率校准都更差。
...
这样一来，原本需要逐层手工推导、既易错又昂贵的偏导数计算，被压缩成一次前向 + 一次反向——这是让深层网络可训练的关键。
