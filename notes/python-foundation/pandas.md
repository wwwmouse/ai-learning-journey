...
| 参数       | 说明           |
| -------- | ---------------- |
| `subset` | 只检查这些列，默认 `None` 检查所有列        |
| `keep`   | `'first'`（默认）：第一次出现标记 `False`，之后重复标记 `True`<br>`'last'`：最后一次出现标记 `False`，之前的重复标记 `True`<br>`False`：所有重复的行都标记 `True` |
...
| 参数             | 说明                              | 默认值       |
| -------------- | ------------------------------- | --------- |
| `subset`       | 按哪些列判断重复，None 表示所有列             | `None`    |
| `keep`         | 保留哪个：`first`(首次) / `last`(末次) / `False`(全删) | `'first'` |
| `inplace`      | 是否原地修改              | `False`   |
| `ignore_index` | 重置索引为 0,1,2...                  | `False`   |
